...
**Status:** Phases 1–9, 11 and 12 are done; Phase 10 is done (§12.10); Phase 13 (scoring & evaluation) is done apart from its analysis and judge-arming tasks, which are `blocked` by design on a stated data precondition; Phase 14 (telemetry fidelity) is done; Phase 15 (post-release follow-ups) is done except one deliberately unnumbered seed-narrowing follow-up; Phase 16 (v2.3.0 shipped features) is done — six product features merged as stacked PRs on 2026-08-31 (S1 secret-exposure alerting, C2 per-team cost anomaly, C4 model routing simulation, E2 session comparison/diff, E3 prompt pattern mining, E4 real-time session stream). Remaining open task statuses are operational sign-off / integration items in P1–P2 plus P6 deferrals superseded by P8
**Owner:** Jorge (SentinelOne)
**Last updated:** 2026-08-31 (see §17 — keep this in step with the last row of the history table)
...
- Real-time alerting / SIEM-style behavioral analytics on session content. *(Update: threshold-based operational alerting is implemented in Phase 9 §12.9. Eight org-level rule types are evaluated — `spend_spike`, `high_error_rate`, `unknown_model_surge`, `autonomy_surge`, `budget_threshold`, `routing_waste`, `disallowed_model` (Phase 10), and `secret_exposure` (Phase 16 S1); plus a per-team `team_spend_spike` rule (Phase 16 C2). Five rules are seeded **disabled** — `budget_threshold`, `routing_waste`, `disallowed_model`, `secret_exposure`, and `team_spend_spike` — with sane defaults, silent until an admin enables them. SIEM-style behavioral analytics on transcript content remains out of scope.)*
...
| `reconcile-cost` | daily when `BILLING_RECONCILIATION_ENABLED=true` | Gated cost reconciliation: compares client-computed `SUM(events.cost_usd)` against the vendor-billed cost for the previous calendar month, per `agent_type`, emitting delta/drift gauges. Vendor cost comes from a composite over whichever vendor sources are configured: `AnthropicBillingSource` (Admin **Cost Report API**, `GET /v1/organizations/cost_report`, `ANTHROPIC_ADMIN_KEY`) for `CLAUDE_CODE`, and `GitHubBillingSource` (**AI-credit usage report**, `GET /{organizations\|enterprises\|users}/{scope}/settings/billing/ai_credit/usage`, `GITHUB_BILLING_TOKEN` + `GITHUB_BILLING_SCOPE`) for `COPILOT`. Falls back to a null source (no comparison) when neither is configured. Every vendor figure is **org-wide** — neither API attributes spend to a developer — so drift is an org-level claim. When the vendor billed and no stored event for that agent carried a token count, the job logs `cost.reconciliation.no_client_token_coverage` and sets the gauges but does **not** count a threshold breach: that drift measures a capture gap, not a pricing error |
...
- `/org/security` — AI-agent data-flow & access posture: tool-category exposure, per-repo exec/network/write exposure, external MCP egress inventory, largest data movements (`tool_output_bytes`), privileged-access audit summary, and a **recent secret exposures** table (Phase 16 S1). Aggregate, visibility-scoped
...
- `/org/prompts` — aggregate prompt pattern mining (Phase 16 E3): clusters user prompts by intent via FTS, shows session reach / cost / friction per cluster. Same trust model as `/org/knowledge` — aggregate, visibility-scoped, small-n suppressed, no conversation content
- `/org/sessions/compare` — side-by-side session comparison/diff (Phase 16 E2): cost, duration, friction, shape, model, tool mix, and PR/CI/review outcomes for two sessions. Each session is independently access-checked and audited
...
39. Alert rules engine — scheduled evaluation of spend spike / error rate / unknown-model thresholds, with persisted firing/resolving history (promotes the render-time anomaly detection of §12.4). Phase 10 added `routing_waste`, `autonomy_surge`, and `disallowed_model`; `budget_threshold` is evaluated too. Phase 16 added `secret_exposure` (S1 — fires when the count of sessions whose shipped transcript matched a redaction class in the trailing 7 days exceeds the configured threshold) and `team_spend_spike` (C2 — fires when any team's 7-day average daily spend exceeds 2.5σ above its own 14-day baseline). Five rules are seeded disabled: `budget_threshold`, `routing_waste`, `disallowed_model`, `secret_exposure`, and `team_spend_spike`; the disabled rules have sane defaults and are silent until an admin enables them, and `budget_threshold` stays inert until a positive budget is configured
...
47. (Phase 16 C4) Routing simulator — `GET /api/org/models/simulate` on `/org/models` lets an operator model the cost impact of shifting tool traffic between model tiers. The pure simulation logic (`simulateRouting`) lives in `packages/schemas/src/model-policy.ts`

**State:** done. Reconciled and closed on 2026-08-20 — `P10-001` through `P10-005` are `done`, `P10-006` is `cancelled` (superseded by `P13-006`'s projection registry). The shared per-agent model policy (`model_policy` table, admin-overridable at `/admin/model-policy`) and the `disallowed_model` alert rule both shipped. Phase 16 C4 added the routing simulator. Per-task evidence is in `tasks/INDEX.md`.
...
### 12.14 Phase 14 — Telemetry Fidelity ✓ done

A seventeen-task correctness pass that proved each fix by reading code and running tests rather than by asserting it. Closed silent gaps in tool-category classification, live turn linkage, cost attribution, and Copilot token capture (settled negatively — the vendor does not persist the event). See `tasks/INDEX.md` for the standing lessons.

### 12.15 Phase 15 — Post-release follow-ups ✓ done (one unnumbered gap)

Opened after v1.0.0 from a seed-and-verify sweep that found three defects where the seed recomputed a value production derives. `P15-001` is `done`; one deliberately unnumbered `tool_action` seed-narrowing follow-up remains open.

### 12.16 Phase 16 — v2.3.0 shipped features ✓ done

Six product features selected from the business-value assessment, merged as stacked PRs on 2026-08-31:
...
See `tasks/INDEX.md` §Phase 16 for per-feature detail.
...
| 2026-08-18 | Jorge (with Claude) | Documentation audit against the code. Corrected the §2.2 and §12.9 claim that `budget_threshold` is "reserved but not evaluated" (all six alert rule types then in the engine are evaluated; two are seeded disabled pending an operator threshold — later expanded to nine types by Phase 10's `disallowed_model` and Phase 16's `secret_exposure` / `team_spend_spike`); narrowed §5.2b's "every human-facing surface filters to `INTERACTIVE`" to what the lints actually enforce, and named the three deliberately-unfiltered read classes; added the four missing `events` columns to §5.3 (`run_kind`, `notification_kind`, `tool_target_hash`, `tool_action`) with their partial indexes; added Phase 13 vocabulary to §16 |
...
| 2026-08-27 | Jorge (with Claude) | Second `reconcile-cost` billing source (P14-017): `GitHubBillingSource`, backed by GitHub's **AI-credit usage report** (`…/settings/billing/ai_credit/usage`, org / enterprise / user scopes, `X-GitHub-Api-Version: 2026-03-10`, classic-PAT auth), for `COPILOT`. Two semantics recorded in §6.5 with it. The vendor figure is an **org-level** aggregate — GitHub publishes no per-user breakdown, `user` is a request filter that is echoed back — so reconciliation makes an org-level claim and no per-developer one. And a vendor bill against an agent that captured **no tokens** is recorded as drift but not as a threshold breach: `events.cost_usd` is derived from tokens, so with none captured the drift is the size of our capture gap, not a pricing error. `COPILOT` is in that state today (P14-007). The source sums `grossAmount`, not `netAmount`, because our price tables are GitHub's published list rates and the invoice figure is post-allowance |
| 2026-08-31 | Jorge (with Claude) | Phase 16 (v2.3.0 shipped features): added §12.16 covering S1 (secret-exposure alerting, #215), C2 (per-team cost anomaly, #216), C4 (model routing simulation, #219), E2 (session comparison/diff, #220), E3 (prompt pattern mining, #221), E4 (real-time session stream, #222). Updated §2.2 alert-rule inventory (eight rule types plus `team_spend_spike`), §12.4 additional pages (`/org/prompts`, `/org/sessions/compare`, `/org/security` recent-exposures table), §12.9 alert rules (`secret_exposure`, `team_spend_spike`), §12.10 state (Phase 10 closed; C4 routing simulator). Added §12.14 (Phase 14) and §12.15 (Phase 15). Updated status header and last-updated date |
