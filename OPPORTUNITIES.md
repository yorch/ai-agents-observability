...
| 4 | **Developer skill progression** (friction trend, shape shift over time) | Medium-High — retention, training, onboarding | Medium | ✅ Friction + shape computed nightly, now as versioned `scores` rows | 🟡 `/me/insights` friction breakdown + recommendations + per-user weekly shape-shift trend; team/org weekly friction trends; org cohort friction divergence by first-seen month. Phase 13 makes the trend honest: a scorer change no longer silently re-bases history, because every score carries the version that produced it. Longitudinal pre/post-adoption still needs baseline data |
...
| 8 | **Code quality correlation** (revert + defect rate by session characteristics) | High — if bug rate correlation holds | High | ✅ Internal PR data ready; Jira bug join shipped via `sync-jira` | 🟡 `/org/quality` shipped (P11-003) with Fisher's exact significance testing on friction-band deltas (P11-004), so a band difference is reported only when it survives a test. Phase 13 adds the missing half — the scorers being correlated now carry a version, so a correlation can say *which* scorer it was computed against. Whether the correlation actually holds is blocked on real data (≥100 outcome-linked PRs) |
| 9 | **Multi-tool comparison** (Claude vs Cursor vs Copilot) | High — procurement decisions | High | ✅ `agent_type` schema ready; seven shipped adapters | 🟡 Comparison table shipped at `/org/agents` (cost/friction/error-rate by `agent_type`). Phase 12 took the seam to seven agents — `claude-code`, `opencode`, `codex`, `gemini-cli`, `copilot`, `pi`, `omp`. Cursor, Aider and Windsurf remain schema entries with no adapter |
