...
|---|---|---|
| Node.js | >=24 (24.6.0 in CI) | Active LTS. The Next.js prod runtime is Node 24. Bun runs everything else. `package.json` uses `"node": ">=24"`; CI pins `24.6.0` exactly via `setup-node@v4`. |
...
- **Pinning policy** (strict):
  ...
  5. Engine pins: `engines.node = ">=24"` in `package.json`; CI pins `node-version: 24.6.0` exactly via `setup-node@v4`. `engines.bun = "1.3.13"` exact. `setup-bun@v2` pins `bun-version: 1.3.13`.
