...
Tiers are **derived** by ranking distinct blended rates inside one agent's own price table, then overlaid with whatever an org admin has set at `/admin/model-policy`. Ranking rather than thresholding is what makes it survive real data: the cheapest-to-dearest spread is ~19× for `claude_code` and ~8000× for `opencode` — whose table P12-012 regenerated from the models.dev catalog, taking it from 34 models to 244 across 20 vendors — so no single multiple could serve both, and the tables deliberately retain retired models whose rates would drag any mean.
