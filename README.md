...
**Phase 13** (scoring & evaluation) — **in progress**. Gives every computed signal provenance and a version (`scores`), separates non-human runs from the human aggregates (`run_kind`), adds content-free trajectory scorers, and captures human session labels. The validation tasks that would calibrate `friction_score` and `shape_label` against real outcomes are `blocked` on a data precondition — no rollout has happened, and calibrating against seed data measures the seed script. See [`tasks/P13-roadmap.md`](./tasks/P13-roadmap.md) and [`docs/research/2026-08-12-llm-evals-assessment.md`](./docs/research/2026-08-12-llm-evals-assessment.md).
