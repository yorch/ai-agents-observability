...
**Phase 10** (model cost optimization) — `done`. P10-001 through P10-006 are `done`: a shared per-agent model policy (tiers derived from the live price tables, admin-overridable at `/admin/model-policy`), price-derived savings **ranges** with volume gating on `/org/models`, team + individual routing and cache coaching, a `disallowed_model` governance alert, and a projected-vs-realized validation loop. Two known limitations are pinned by tests rather than hidden: retired models can surface as downgrade targets, and current Opus tiers as `standard` because the price tables retain its retired `$15/$75` rows. See [`tasks/P10-roadmap.md`](./tasks/P10-roadmap.md).
