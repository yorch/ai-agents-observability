...
// SHA-1(namespace || name), first 16 bytes with variant bits set.
// Version nibble is left for callers to set.
function sha1Bytes(name: string, namespace: string = IMPORT_NAMESPACE): Buffer {
...
export function uuidv5(name: string, namespace: string = IMPORT_NAMESPACE): string {
  const bytes = sha1Bytes(name, namespace);
