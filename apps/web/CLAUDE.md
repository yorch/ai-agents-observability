...
- **Semantic tokens only — never raw palette utilities.** `bg-white/5`, `text-red-400`, `border-yellow-500/40` and friends are dark-only by construction and silently break the light theme; there are zero left in the app and none should come back. Use:
  ...
  - series `series-1 … series-6` — charts only, assigned by entity in fixed order
...
- **Build from `src/components/ui`.** `Card`, `Stat`, `Table`/`Row`/`Cell`, `Badge`, `PageHeader`, `SectionHeader`, `EmptyState`, and the chart set (`BarChart`, `HBars`, `Sparkline`, `Legend`, `ChartHover`, `scale`). Don't hand-roll a card, stat tile or table — that duplication is what this layer replaced.
