...
- **Build from `src/components/ui`.** `Card`, `Stat`, `Table`/`Row`/`Cell`, `Badge`/`SeriesBadge`, `Button`/`ButtonLink`, `Input`/`Select`/`Field`, `Segmented`, `PageHeader`, `SectionHeader`, `EmptyState`, `SkeletonCard`/`SkeletonBar`, and the chart set (`BarChart`, `HBars`, `AreaLine`, `Sparkline`, `Legend`, `ChartHover`, `scale`). Reach for these rather than hand-rolling a card, button, control or table.
  - **Migration is partial.** 36 inline card blocks and 46 raw `<table>`s still predate the primitives. They are not a second convention to follow — when you touch one of those files, move it onto the primitive. The remaining tables need converting by hand: their headers carry dynamic labels, colSpans and conditional cell classes that a codemod loses.
