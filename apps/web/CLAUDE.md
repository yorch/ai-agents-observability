...
- **Build from `src/components/ui`.** `Card`, `Stat`, `Table`/`Row`/`Cell`, `Badge`/`SeriesBadge`, `Button`/`ButtonLink`, `Input`/`Select`/`Field`, `Segmented`, `Pagination`, `PageHeader`, `SectionHeader`, `EmptyState`, `SkeletonCard`/`SkeletonBar`, and the chart set (`BarChart`, `HBars`, `AreaLine`, `ShareBar`, `Sparkline`, `Legend`, `ChartHover`, `scale`). Reach for these rather than hand-rolling a card, button, control or table.
  ...
  - **Still outstanding:** 34 inline card blocks whose padding differs from `Card`'s `p-4`, and 13 `<select>`s whose padding matches neither control size. Both need a sizing decision rather than a codemod — make it when you touch them.
