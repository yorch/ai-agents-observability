...
export function TeamSubNav({
  ...
  active: 'overview' | 'roster' | 'sessions' | 'adoption' | 'tools' | 'mcp' | 'skills' | 'prs';
  slug: string;
}) {
  ...
  return (
    <nav className="mb-6 flex gap-6 border-b border-white/10">
      ...
      <Link href={`/team/${slug}/sessions`} className={linkClass('sessions')}>
        Sessions
      </Link>
      <Link href={`/team/${slug}/adoption`} className={linkClass('adoption')}>
        Adoption
      </Link>
      <Link href={`/team/${slug}/tools`} className={linkClass('tools')}>
        Tools
      </Link>
      <Link href={`/team/${slug}/mcp`} className={linkClass('mcp')}>
        MCP
      </Link>
