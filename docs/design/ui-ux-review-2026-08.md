...
> **Post-review hardening (2026-08-18, this branch):** a follow-up `/code-review` + `/simplify` pass over the implementation itself surfaced and fixed a second round: the command palette now portals its overlay (it opened invisibly below `lg`), `useFocusTrap` filters to visible focusables, owns Escape-to-close (one layer at a time) and skips focus restore after click-outside closes, admin actions are wrapped in `withActionResult` so unexpected throws render inline instead of crashing the page, `/me/sessions` distinguishes an out-of-range page from first-run, the jobs `queued` badge no longer masks a failed last run, clipboard errors render where the Copy button lives, date formatters are cached module-level `Intl.DateTimeFormat`s with a shared `fmtDayShort`, `/me` streams without a query waterfall, the four copy-pasted error/loading boundaries became shared `SectionError`/`SectionLoading` re-exports, in-card empties became a `CardEmpty` primitive, the org search pager moved onto `Pagination`, per-session timestamps carry a UTC hint, and the fonts are vendored (`next/font/local`) so builds have no Google Fonts network dependency.
>
