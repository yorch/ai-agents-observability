...
The `claude-telemetry` hook binary captures events from seven AI coding agents.
...
| GitHub Copilot CLI | `copilot` | `~/.copilot/hooks/claude-telemetry.json` | no |
...
```bash
curl -fsSL https://raw.githubusercontent.com/yorch/ai-agents-observability/main/scripts/install-hook.sh | bash -s -- --prefix "$HOME/.local/bin"
```
...
|---|---|
| macOS (Apple Silicon) | `claude-telemetry-darwin-arm64` |
| macOS (Intel) | `claude-telemetry-darwin-x64` |
| Linux (ARM64) | `claude-telemetry-linux-arm64` |
| Linux (x86-64) | `claude-telemetry-linux-x64` |
...
`~/.claude-telemetry/identity.json`.
...
# For any other agent
claude-telemetry install --agent codex
claude-telemetry install --agent gemini-cli
claude-telemetry install --agent copilot
claude-telemetry install --agent pi
claude-telemetry install --agent omp
claude-telemetry install --agent opencode
```
...
> **The snippets below are generated by `claude-telemetry install --agent <name>`.
...
```json
{
  "hooks": {
    "Notification": [
      { "hooks": [{ "args": ["hook", "notification"], "command": "/usr/local/bin/claude-telemetry", "type": "command" }] }
    ],
    "PostToolUse": [
      { "hooks": [{ "args": ["hook", "post-tool-use"], "command": "/usr/local/bin/claude-telemetry", "type": "command" }] }
    ],
    "PreCompact": [
      { "hooks": [{ "args": ["hook", "pre-compact"], "command": "/usr/local/bin/claude-telemetry", "type": "command" }] }
    ],
    "PreToolUse": [
      { "hooks": [{ "args": ["hook", "pre-tool-use"], "command": "/usr/local/bin/claude-telemetry", "type": "command" }] }
    ],
    "SessionStart": [
      { "hooks": [{ "args": ["hook", "session-start"], "command": "/usr/local/bin/claude-telemetry", "type": "command" }] }
    ],
    "Stop": [
      { "hooks": [{ "args": ["hook", "stop"], "command": "/usr/local/bin/claude-telemetry", "type": "command" }] }
    ],
    "SubagentStop": [
      { "hooks": [{ "args": ["hook", "subagent-stop"], "command": "/usr/local/bin/claude-telemetry", "type": "command" }] }
    ],
    "UserPromptSubmit": [
      { "hooks": [{ "args": ["hook", "user-prompt-submit"], "command": "/usr/local/bin/claude-telemetry", "type": "command" }] }
    ]
  }
}
```

Replace `/usr/local/bin/claude-telemetry` with the actual path to your binary
...
**Default (notify wrapper):** Save this as `~/.codex/claude-telemetry-notify.sh`
and `chmod +x` it:

```sh
#!/bin/sh
printf '%s' "$1" | /usr/local/bin/claude-telemetry hook turn-complete --agent codex
```

Then point Codex at the wrapper in `~/.codex/config.toml`:

```toml
notify = ["/home/user/.codex/claude-telemetry-notify.sh"]
```
...
then re-run `claude-telemetry install --agent codex` and write the output to
...
```json
{
  "hooks": {
    "AfterAgent": [
      { "hooks": [{ "command": "\"/usr/local/bin/claude-telemetry\" hook after-agent --agent gemini-cli", "name": "claude-telemetry-after-agent", "timeout": 5000, "type": "command" }] }
    ],
    "AfterModel": [
      { "hooks": [{ "command": "\"/usr/local/bin/claude-telemetry\" hook after-model --agent gemini-cli", "name": "claude-telemetry-after-model", "timeout": 5000, "type": "command" }] }
    ],
    "AfterTool": [
      { "hooks": [{ "command": "\"/usr/local/bin/claude-telemetry\" hook after-tool --agent gemini-cli", "name": "claude-telemetry-after-tool", "timeout": 5000, "type": "command" }] }
    ],
    "BeforeAgent": [
      { "hooks": [{ "command": "\"/usr/local/bin/claude-telemetry\" hook before-agent --agent gemini-cli", "name": "claude-telemetry-before-agent", "timeout": 5000, "type": "command" }] }
    ],
    "BeforeTool": [
      { "hooks": [{ "command": "\"/usr/local/bin/claude-telemetry\" hook before-tool --agent gemini-cli", "name": "claude-telemetry-before-tool", "timeout": 5000, "type": "command" }] }
    ],
    "Notification": [
      { "hooks": [{ "command": "\"/usr/local/bin/claude-telemetry\" hook notification --agent gemini-cli", "name": "claude-telemetry-notification", "timeout": 5000, "type": "command" }] }
    ],
    "PreCompress": [
      { "hooks": [{ "command": "\"/usr/local/bin/claude-telemetry\" hook pre-compress --agent gemini-cli", "name": "claude-telemetry-pre-compress", "timeout": 5000, "type": "command" }] }
    ],
    "SessionEnd": [
      { "hooks": [{ "command": "\"/usr/local/bin/claude-telemetry\" hook session-end --agent gemini-cli", "name": "claude-telemetry-session-end", "timeout": 5000, "type": "command" }] }
    ],
    "SessionStart": [
      { "hooks": [{ "command": "\"/usr/local/bin/claude-telemetry\" hook session-start --agent gemini-cli", "name": "claude-telemetry-session-start", "timeout": 5000, "type": "command" }] }
    ]
  }
}
```
...
Write to `~/.copilot/hooks/claude-telemetry.json`:

```json
{
  "disableAllHooks": false,
  "hooks": {
    "agentStop": [{ "command": ["/usr/local/bin/claude-telemetry", "hook", "agent-stop", "--agent", "copilot"], "timeoutSec": 5, "type": "command" }],
    "notification": [{ "command": ["/usr/local/bin/claude-telemetry", "hook", "notification", "--agent", "copilot"], "timeoutSec": 5, "type": "command" }],
    "postToolUse": [{ "command": ["/usr/local/bin/claude-telemetry", "hook", "post-tool-use", "--agent", "copilot"], "timeoutSec": 5, "type": "command" }],
    "postToolUseFailure": [{ "command": ["/usr/local/bin/claude-telemetry", "hook", "post-tool-use-failure", "--agent", "copilot"], "timeoutSec": 5, "type": "command" }],
    "preCompact": [{ "command": ["/usr/local/bin/claude-telemetry", "hook", "pre-compact", "--agent", "copilot"], "timeoutSec": 5, "type": "command" }],
    "preToolUse": [{ "command": ["/usr/local/bin/claude-telemetry", "hook", "pre-tool-use", "--agent", "copilot"], "timeoutSec": 5, "type": "command" }],
    "sessionEnd": [{ "command": ["/usr/local/bin/claude-telemetry", "hook", "session-end", "--agent", "copilot"], "timeoutSec": 5, "type": "command" }],
    "sessionStart": [{ "command": ["/usr/local/bin/claude-telemetry", "hook", "session-start", "--agent", "copilot"], "timeoutSec": 5, "type": "command" }],
    "subagentStop": [{ "command": ["/usr/local/bin/claude-telemetry", "hook", "subagent-stop", "--agent", "copilot"], "timeoutSec": 5, "type": "command" }],
    "userPromptSubmitted": [{ "command": ["/usr/local/bin/claude-telemetry", "hook", "user-prompt-submitted", "--agent", "copilot"], "timeoutSec": 5, "type": "command" }]
  },
  "version": 1
}
```
  ...
  for (const [native, kind] of Object.entries(KINDS)) {
    pi.on(native, async (event: any, ctx: any) => {
      try {
        ...
        const p = spawn("/usr/local/bin/claude-telemetry", ['hook', kind, '--agent', 'pi'], {
  ...
  for (const [native, kind] of Object.entries(KINDS)) {
    omp.on(native, async (event: any, ctx: any) => {
      try {
        ...
        const p = spawn("/usr/local/bin/claude-telemetry", ['hook', kind, '--agent', 'omp'], {
          stdio: ['pipe', 'ignore', 'ignore'],
          detached: true,
        });
        p.stdin.end(JSON.stringify(payload));
...
// "/usr/local/bin/claude-telemetry hook <kind> --agent omp" there instead. We ship the native module
...
export const telemetry: Plugin = async () => ({
  event: async ({ event }) => {
    ...
    if (!kind) return;
    const p = Bun.spawn(['/usr/local/bin/claude-telemetry', 'hook', kind, '--agent', 'opencode'], { stdin: 'pipe' });
...
Authentication (`claude-telemetry login`) is required unless `--dry-run` is
...
| Command | Description |
|---|---|
| `claude-telemetry status` | Show auth status, queue depth, service state |
| `claude-telemetry pause` | Temporarily stop sending telemetry |
| `claude-telemetry resume` | Re-enable telemetry |
| `claude-telemetry uninstall` | Remove hooks and background services |
| `claude-telemetry purge-local` | Delete all local data (queue, logs, identity) |
