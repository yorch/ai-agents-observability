...
| **Alert rules engine** — scheduled, idempotent, **aggregate-only** evaluation of spend-spike / high-error-rate / unknown-model thresholds | ✅ | 3 seeded rules at the time of writing, 7 today ([0001_init.sql](../../packages/db/sql/migrations/0001_init.sql), which the 2026-08-14 squash folded the original `0002_seed_builtin_alert_rules.sql` into); thresholds in [packages/schemas/src/alerts.ts](../../packages/schemas/src/alerts.ts); job in `apps/ingest/src/jobs/evaluate-alerts.ts` |
