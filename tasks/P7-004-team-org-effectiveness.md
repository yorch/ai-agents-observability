...
- [ ] `/team/[slug]` shows a friction distribution panel (histogram or percentile bar: p25/p50/p75) for the trailing 30d, using only sessions from members whose `share_metadata_with_team` flag is set.
- [ ] `/team/[slug]` shows a shape-mix panel (proportional bar chart) for the same population and period.
- [ ] `/org/dashboard` shows an org-level friction distribution and shape mix for the trailing 30d, using only sessions from members whose `share_metadata_with_org` flag is set.
- [ ] No individual member's friction score or session shape is surfaced to a user holding only the `viewer_aggregate` role; only bucketed/aggregate statistics are shown.
- [ ] Panels render `FRICTION_VERSION` and a "not enough data" notice when fewer than 5 scored sessions are in the aggregate (to avoid identifying individuals by elimination).
- [ ] Null-score sessions are excluded from distribution calculations (not counted as 0).
